// src/components/campeonatos/EditCampeonatoModal.tsx
'use client'
...
import { Trophy, Building2, MapPin, Users, Save, Hash, Medal } from 'lucide-react'
import type { Campeonato, CampeonatoInsert } from '@/types/campeonato'

interface EditCampeonatoModalProps {
  isOpen: boolean
  onClose: () => void
  onSubmit: (id: string, data: Partial<CampeonatoInsert>) => void
...
export function EditCampeonatoModal({
  isOpen,
  onClose,
  onSubmit,
  campeonato,
}: EditCampeonatoModalProps) {
  const [formData, setFormData] = useState({
    logo: campeonato.logo_url ?? '',
    nome: campeonato.nome,
    organizador: campeonato.organizador,
    local: campeonato.local,
    telefone: campeonato.telefone ?? '',
    codigoPais: campeonato.phone_code ?? '+55',
    ...
    anoCriacao: campeonato.ano_criacao?.toString() ?? '',
    numeroClassificados: campeonato.numero_classificados ?? 0,
  })

  // Atualiza o form se trocar o campeonato selecionado
  useEffect(() => {
    setFormData({
      logo: campeonato.logo_url ?? '',
      nome: campeonato.nome,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const payload: Partial<CampeonatoInsert> = {
      nome: formData.nome,
      ...
      numero_classificados: formData.numeroClassificados > 0 ? formData.numeroClassificados : undefined,
    }

    onSubmit(campeonato.id, payload)
    onClose()
  }

  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Editar Campeonato" size="2xl">
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          {/* Classificados — só para formatos com mata-mata */}
          {(formData.formato === 'Pontos Corridos' || formData.formato === 'Grupos + Mata-Mata') && (
            <div className="col-span-1 sm:col-span-2">
              <Input
                label="Times classificados para Mata-Mata"
                type="number"
                placeholder="Ex: 16, 8 ou 4"
                value={formData.numeroClassificados > 0 ? formData.numeroClassificados.toString() : ''}
                onChange={(e) => set('numeroClassificados', parseInt(e.target.value) || 0)}
                icon={Medal}
                iconPosition="left"
                min={2}
                max={formData.numeroTimes}
                hint="Quantos times avançam para o mata-mata? (Ex: 16, 8, 4)"
              />
            </div>
          )}
        </div>
