...
import {
  ...
  History, BarChart3, Target, Settings, Bell, Plus, LogOut, ChevronDown,
} from 'lucide-react'
...
const navigation = [
  { name: 'Dashboard',          href: '/dashboard',     icon: LayoutDashboard, section: NAV_SECTION    },
  { name: 'Campeonatos',        href: '/campeonatos',   icon: Trophy,          section: NAV_SECTION    },
  { name: 'Times',              href: '/times',         icon: Users,           section: NAV_SECTION    },
  { name: 'Jogadores',          href: '/jogadores',     icon: UserCircle,      section: NAV_SECTION    },
  { name: 'Jogos',              href: '/jogos',         icon: Calendar,        section: NAV_SECTION    },
  { name: 'Histórico de Jogos', href: '/historico',     icon: History,         section: MANAGE_SECTION },
  { name: 'Classificação',      href: '/classificacao', icon: BarChart3,       section: MANAGE_SECTION },
  { name: 'Artilharia',         href: '/artilharia',    icon: Target,          section: MANAGE_SECTION },
  { name: 'Configurações',      href: '/configuracoes', icon: Settings,        section: SYSTEM_SECTION },
  { name: 'Notificações',       href: '/notificacoes',  icon: Bell,            section: SYSTEM_SECTION },
]
...
export function Sidebar({ user }: SidebarProps) {
  ...
  // ✅ Completamente hidden no mobile — só aparece em lg+
  return (
      ...
      <div className="border-t border-gray-200 p-3 shrink-0">
        <Button variant="ghost" size="sm" fullWidth onClick={() => setUserMenuOpen(!userMenuOpen)} className="justify-start! gap-3 px-2!">
          ...
          <div className="flex-1 text-left min-w-0">
            ...
            <p className="text-xs text-gray-500">{user.role === 'admin' ? 'Administrador' : 'Técnico'}</p>
          </div>
